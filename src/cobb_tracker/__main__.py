...
def main():
    ...
    if args.push_to_database is False and args.municipality is None and args.pull_all_cities is None:
        logging.error("You must specify -m/--municipality if -p/--push-to-database is not selected")
        sys.exit()

    if args.pull_all_cities is True and args.municipality is not None:
        logging.error("-m/--municipality and -a/--pull-all-cities are mutually exclusive")
        sys.exit()
