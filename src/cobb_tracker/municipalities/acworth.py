...
def get_all_events(session: requests.Session) -> list:
    ...
    def get_event(group: dict, years: int, session: requests.Session):
        group_id = group["ID"]
        for year in years:
            event_page = json.loads(session.get(f"{MEETINGS_URL}Range={year}&Group={group_id}/",
                                                headers={"User-Agent": USER_AGENT}).text)
            event_list.append(event_page)

    startup_page = json.loads(session.get(STARTUP_URL,
                                            headers={"User-Agent": USER_AGENT}).text)
    ...
    with cf.ThreadPoolExecutor(max_workers=15) as executor:
        future_groups = {executor.submit(get_event, group, years, session): group for group in meeting_groups}
