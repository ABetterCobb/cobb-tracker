...
def get_minutes_docs(config: CobbConfig):
    ...
    try:
        ...
        start_date = browser.find_element(
            "id",
            "ctl00_ContentPlaceHolder1_SearchAgendasMeetings_radCalendarFrom_dateInput",
        )
        start_date.clear()
        ...
        start_date = browser.find_element(
            "id",
            "ctl00_ContentPlaceHolder1_SearchAgendasMeetings_radCalendarTo_dateInput",
        )
        start_date.clear()
        ...
        minutes_row_reg = re.compile(
            r"ctl00_ContentPlaceHolder1_SearchAgendasMeetings_radGridMeetings_ctl00.*"
        )
        minutes_link_reg = re.compile(
            r"ctl00_ContentPlaceHolder1_SearchAgendasMeetings_radGridMeetings.*hypMinutesPDF"
        )
        ...
        while True:
            ...
            all_rows = [
                row
                for row in browser.find_elements(By.TAG_NAME, "tr")
                if minutes_row_reg.search(row.get_attribute("id"))
            ]
            for row in all_rows:
                columns = [
                    column.text
