    ...
    for select_tag in archive_details.find_all("select"):
        for label in archive_details.find_all("label"):
            if label.get("for") == select_tag.get("id"):
                ...
                group_id = re.sub(r"[a-zA-Z]", "", select_tag.get("id"))
...
def get_meeting_info(archive_groups: dict):
    ...
    for group in archive_groups.keys():
        if (
            archive_groups[group] != "Press Releases"
            and archive_groups[group] != "Town Hall Meetings and Retreats"
            and archive_groups[group] != "City Council Work Session"
            and "Agenda" not in archive_groups[group]
        ):
            group_page = f"{LIST_OF_ARCHIVE_SECTIONS}?AMID={group}&Type=&ADID="
            archive_page_response = session.get(
                group_page, headers={"User-Agent": USER_AGENT}
            )

            if not archive_page_response.ok:
                logging.error(
                    "Request failed:",
                    archive_page_response.reason,
                    archive_page_response.status_code,
                )
                return
            ...
            for span in all_meetings:
                ...
                if link is not None and name is not None:
                    if "agenda" not in name.text.lower():
                        ...
                        minutes_urls[file_url]["meeting_name"] = (
                            re.sub(r"\s?Minutes", "", archive_groups[group])
                            .strip()
                            .replace(" ", "_")
                        )
                        ...
                        if date is not None:
                            ...
                            if date is not None:
                                ...
                                if not singular_page.ok:
                                    logging.error(
                                        "Request failed:",
                                        singular_page.reason,
                                        singular_page.status_code,
                                    )
                                    return
                                singular_page_text = BeautifulSoup(
                                    singular_page.text, "html.parser"
                                )
                                ...
                                for link in links:
                                    if link.get("href") is not None:
                                        if "ArchiveCenter" in (
                                            ...
                                            date = get_year.search(
                                                filtered_link_text[0]
                                            )

                                            if date is not None:
                                                minutes_urls[file_url][
                                                    "date"
                                                ] = parse_date(date.group(0))
